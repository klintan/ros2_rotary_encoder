...
RotaryEncOverMCP rotaryEncoders[] = {
  ...
  RotaryEncOverMCP(&mcp, 14, 15, &RotaryEncoderChanged, 1),
...
void setup() {
  ...
  Serial.println("initialize: " + String(numEncoders));
  ...
  attachInterrupt(digitalPinToInterrupt(arduinoIntPin), intCallBack, RISING);
}

// The int handler will just signal that the int has happened
// we will do the work from the main loop.
void intCallBack() {
  Serial.println("Callback intCallBack");
